...
window.getInvestorName = function(user) {
    Investor.deployed().then(function(instance) { 
        return instance.getInvestorName(user) }).then(function(result) {
            ...
            document.getElementById("tokenAddress").value = ethAddress;
        }) 
}
...
window.redeem = function() {
    ...
    console.log(addr);
    var redeemTokens = document.getElementById("redeemTokens").value;
    getUserRole("userrole", function(node) {
        Asset.deployed().then(function(instance) {
        console.log(redeemTokens);
        return instance.redeemAssetCard(redeemTokens, {from:ethAddress, gas: 600000, privateFor: ["IbTNgT4uQiGfqQMMz1gfggc9fsq3OTgERqzpTLrR0Es=", node.node1, node.node2, node.node3, node.node4, node.node5]}) }).then(function(result) {
            console.log(result);
            Zerv.deployed().then(function(instance) { 
            return instance.redeemTokens(ethAddress, redeemTokens, {from:ethAddress,gas: 300000, privateFor: ["IbTNgT4uQiGfqQMMz1gfggc9fsq3OTgERqzpTLrR0Es=", node.node1, node.node2, node.node3, node.node4, node.node5]}) }).then(function(receipt) {
                console.log(receipt);
                location.reload();
            })
        })
